...
SERVICE_ACCOUNT_FILE = os.path.join(BASE_DIR, 'sales', 'credentials.json')

SCOPES = ['https://www.googleapis.com/auth/spreadsheets']
SPREADSHEET_ID = '1xB20JwR0GU37i66zJnbLrBZXteWcoTGPlTtQ0hsaIbQ'
SHEET_NAME = 'Продажа'
SHEET_NAME1 = 'Диспетчер'
NACHISLENIE_SHEET_NAME = 'Начисление'  # New sheet name
...
def append_to_nachislenie_sheet(data):
    """Append data to the Начисление sheet, starting from the first column (A)."""
    try:
        creds = Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE, scopes=SCOPES)
        service = build('sheets', 'v4', credentials=creds)
        sheet = service.spreadsheets()

        # Set the range to start at A, which is the first column.
        # This means the data will be inserted starting from column A.
        range_name = f'{NACHISLENIE_SHEET_NAME}!B:J'
